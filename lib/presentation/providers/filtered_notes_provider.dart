...
@riverpod
Future<List<Note>> mainNotes(ref) async {
  final notes = await ref.watch(notesProvider.future);
  return notes.where((note) => !note.isDeleted && !note.isArchived).toList();
}
...
Future<List<Note>> favoriteNotes(ref) async {
  final notes = await ref.watch(notesProvider.future);
  return notes
      .where((note) => note.isFavorite && !note.isDeleted && !note.isArchived)
      .toList();
}
...
Future<List<Note>> trashedNotes(ref) async {
  final List<Note> notes = await ref.watch(notesProvider.future);
